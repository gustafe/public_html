  ...
  function set_extended_data($file_path) {
    ...
    if(isset($info["APP13"])) {                         
      ...
      if(isset($iptc["2#005"][0]))
        $this->data['@title'] = iconv("ISO-8859-1", "UTF-8//TRANSLIT",$iptc["2#005"][0]);
      # @description
      if(isset($iptc["2#120"][0])) 
        $this->data['@description'] = iconv("ISO-8859-1", "UTF-8//TRANSLIT",$iptc["2#120"][0]);
      # @keywords
...
# get @description from a txt file
    $description_path = preg_replace('/(\.[\w\d]+?)$/', '.dsc', $file_path);
